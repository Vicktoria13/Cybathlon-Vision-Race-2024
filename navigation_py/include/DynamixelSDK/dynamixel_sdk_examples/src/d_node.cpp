...
: Node("Moteur")
{
  ...
  get_pos_publisher_ = this->create_publisher<SetPosition>("current_position",10);
  timer_ = this->create_wall_timer(
    50ms,std::bind(&ReadWriteNode::timer_callback,this));
  set_position_subscriber_ =
    ...
    [this](const SetPosition::SharedPtr msg) -> void
    {
      ...
      uint32_t velocity = (unsigned int)msg->position;  // Convert int32 -> uint32
...
{     
      SetPosition message = SetPosition();
      message.id = 1;
            dxl_comm_result = packetHandler->read4ByteTxRx(
        portHandler,
        (uint8_t) message.id,
        ADDR_PRESENT_POSITION,
        reinterpret_cast<uint32_t *> (&message.position),
        &dxl_error
      );
      RCLCPP_INFO(
        this->get_logger(),
        "Get [ID: %d] [Present Position: %d]",
        message.id,
        message.position-3270
      );
      get_pos_publisher_->publish(message);
      message.id = 2;
      dxl_comm_result = packetHandler->read4ByteTxRx(
        portHandler,
        (uint8_t) message.id,
